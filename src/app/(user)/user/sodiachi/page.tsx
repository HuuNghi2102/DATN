...
export default function AccountPage() {
    ...
    const handleDeleteAddress = async (id: number) => {
        ...
        if (accessTokenLocal && typeTokenLocal) {
            try {
                const response = await fetch(`https://huunghi.id.vn/api/address/deleteAddress/${id}`, {
                    method: 'DELETE',
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": `${JSON.parse(typeTokenLocal)} ${JSON.parse(accessTokenLocal)}`
        ...
        if (accessTokenLocal && typeTokenLocal) {
            try {
                ...
                if (isEditing) {
                    ...
                    response = await fetch(`https://huunghi.id.vn/api/address/updateAddress/${addressForm.id_dia_chi_giao_hang}`, {
                        method: 'PUT',
                        headers: {
                            "Content-Type": "application/json",
                            "Authorization": `${JSON.parse(typeTokenLocal)} ${JSON.parse(accessTokenLocal)}`
                    ...
                    // Add new address
                    response = await fetch('https://huunghi.id.vn/api/address/addAddress', {
                        method: 'POST',
                        headers: {
                            "Content-Type": "application/json",
                            "Authorization": `${JSON.parse(typeTokenLocal)} ${JSON.parse(accessTokenLocal)}`
                ...
                if (result.status == false && result.errors) {
                    setErrFormAddress({
                        ten_nguoi_nhan: result.errors?.name?.length > 0 ? result.errors.name[0] : '',
                        so_dien_thoai_nguoi_nhan: result.errors?.phone?.length > 0 ? result.errors.phone[0] : '',
                        dia_chi_nguoi_nhan: result.errors?.address?.length > 0 ? result.errors?.address[0] : '',
                    })
                    return;
                }
                                ...
                                <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                        <div className="flex items-center space-x-3 mb-2">
                                            ...
                                            <button
                                                onClick={() => handleDeleteAddress(address.id_dia_chi_giao_hang)}
