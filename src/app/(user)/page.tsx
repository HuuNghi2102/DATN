...
const Home = () => {
  const [productsBestSaler, setProductsBestSaler] = useState<
    productsBestSalerInterface[]
  >([]);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          "https://huunghi.id.vn/api/categoryProduct/getCateAndProducts"
        );
        const result = await res.json();
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          "https://huunghi.id.vn/api/product/getProductNews"
        );
        const result = await res.json();
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          "https://huunghi.id.vn/api/product/getProductSales"
        );
        const result = await res.json();
    ...
    const fetchBanner = async () => {
      try {
        let getAPIbanner = await fetch(
          "https://huunghi.id.vn/api/banner/getBannerByPage"
        );
    ...
    <div>
      <div className="container  mx-auto lg:pt-[12%] pt-[25%] max-w-[1200px] px-4">
              ...
              {voucher.map((voucher, index) => (
                ...
                >
                  <div className="flex items-center justify-center w-[80px] font-bold text-sm">
                    <span className="text-xs mr-2">{voucher.ma_giam_gia}</span>
                  </div>
                  <div className="text-xs border-l border-dashed border-black pl-2 relative w-full">
                    <div className="py-1 pr-2">
                      <h4 className="font-semibold">
                        ĐƠN HÀNG:{" "}
                        {typeof voucher.gia_tri_don_hang === "number"
        ...
        <div className="my-4">
          ...
          <div>
            <img
              src="../assets/images/yptvddzi.jpg"
              alt="Best Seller"
          ...
          <div>
            <img
              src="/assets/images/banchay_a01333a0db53411883d51490d22b7eab.webp"
              alt="Best Seller"
              className="w-full object-cover rounded-lg"
            />
            <Slider {...productSettings} className="my-4">
        ...
        <div className="comboMix py-6">
          <h2 className="text-xl font-semibold mb-4 uppercase sm:text-lg">
            COMBO MIX & MATCH ĐÚNG CHUẨN
          </h2>
