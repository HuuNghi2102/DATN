...
const PaymentSuccess = () => {
  ...
  const fetchOrder = async () => {
    ...
    if(accessTokenLocal && typeTokenLocal && userLocal){
      const res = await fetch(`https://huunghi.id.vn/api/order/getOrder/${id}`,{
        headers : {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-800 to-black px-4 pt-[11%]">
      <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center">
        ...
        {order && (
          <div className="mb-2 text-gray-700">
            <strong>Tổng đơn hàng:</strong> {(order.gia_tong_don_hang + order.tien_ship).toLocaleString('vi-VN')}đ
          </div>
        )}
