      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-2 mb-4 z-0">
        {currentProducts.map((product) => (
          <div key={product.id} className="bg-white p-2 rounded border border-gray-200 group relative">
            <div className="relative">
              <img alt={product.name} src={product.image} className="w-full h-auto rounded" loading="lazy" />
              <img alt={`${product.name} alternate`} src={product.hoverImage} className="absolute top-0 left-0 w-full h-auto rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10" loading="lazy" />
              ...
              {product.label && (
                <div className={`absolute top-2 right-2 px-2 py-[2px] text-[11px] font-semibold rounded z-30 ${['Sieu Mat', 'Sieu Nhe'].includes(product.label) ? 'text-white bg-blue-600' : 'text-black bg-orange-500'}`}>
                  {product.label}
                </div>
              )}

              <a href="#" className="absolute right-2 bottom-2 bg-black w-7 h-7 rounded-full flex justify-center items-center text-white text-sm hover:bg-white hover:text-black z-30">
            ...
            {product.discount ? (
              <div className="flex items-center gap-2 mt-0.5 leading-[1.25rem]">
                ...
                <span className="bg-red-600 text-white text-[11px] font-bold px-1.5 py-[1px] rounded">{product.discountPercent}</span>
              </div>
            ) : (
