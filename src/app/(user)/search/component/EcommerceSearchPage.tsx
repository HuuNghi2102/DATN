...
const EcommerceSearchPage: React.FC = () => {
  ...
  const fetchData = async () => {
    const resData = await fetch(
      `https://huunghi.id.vn/api/product/getSearch?page=${currentPage}&type=${activeTab}&search=${search}`
    );
    ...
    setPageEnd(
      currentPage + 2 >= result.data.results.last_page
        ? result.data.results.last_page
        : currentPage + 2
    );
