  ...
  const fetchReviews = async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://huunghi.id.vn/api/evaluate/listEvaluate?start=${selectedStart}&idProduct=${selectedIdProduct}&date=${selectedDate}`,
        {
          headers: {
      ...
      const rawData = Array.isArray(result?.data?.evaluates.data)
        ? result?.data?.evaluates.data
        : [];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          ...
          <div>
            ...
            >
              ...
              {listProduct.map((e, i) => (
                <option key={i} value={e.id_san_pham}>
                  {e.ten_san_pham}
                </option>
              ))}
            </select>
          ...
          <div className="px-4 py-3 flex items-center justify-between border-t border-gray-200">
            <div className="text-sm text-gray-700">
              ...
              <span className="font-medium">
                {Math.min(
                  currentPage * perPage,
                  (currentPage - 1) * perPage + reviews.length
                )}
              </span>{" "}
              trong tổng số{" "}
              <span className="font-medium">{totalPages * perPage}</span> đánh
              giá
            </div>
