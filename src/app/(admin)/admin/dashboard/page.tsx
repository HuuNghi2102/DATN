...
const Dashboard = () => {
  ...
  const fetchDefaultData = async () => {
    ...
    if(accessTokenLocal && typeTokenLocal && userLocal) {
      ...
      if (user.id_vai_tro == 1) {
        //
        const doanhThuThang = await fetch('https://huunghi.id.vn/api/function/baocao',{
          headers : {
        ...
        if(doanhThuThang.ok){
          ...
          const getOk = result.data.doanhThu.map((e:any) => ({ month:'Tháng '+ e.month , revenue : e.revenue  }))
        ...
        //
        const resQuantityOfOrderForStatusOrder = await fetch('https://huunghi.id.vn/api/function/quantityOfOrderForStatusOrder',{
          headers : {
            "Content-Type" : "application/json",
            "Authorization" : `${JSON.parse(typeTokenLocal)} ${JSON.parse(accessTokenLocal)}`
          }
        ...
        if(resQuantityOfOrderForStatusOrder.ok){
          const resultResQuantityOfOrderForStatusOrder = await resQuantityOfOrderForStatusOrder.json();
          const getOk = [
            { name: 'Thành công', value: resultResQuantityOfOrderForStatusOrder.data.success},
            { name: 'Đang xử lý', value: resultResQuantityOfOrderForStatusOrder.data.processing },
            { name: 'Đang giao', value: resultResQuantityOfOrderForStatusOrder.data.inProgress },
            { name: 'Đã hủy', value: resultResQuantityOfOrderForStatusOrder.data.destroy },
          ]
        ...
        //
        const resProductBestSeller = await fetch('https://huunghi.id.vn/api/product/getBestSalerProducts',{
          headers : {
            "Content-Type" : "application/json",
            "Authorization" : `${JSON.parse(typeTokenLocal)} ${JSON.parse(accessTokenLocal)}`
          }
        ...
        if(resProductBestSeller.ok){
          ...
          setProductBestSeller(resultResProductBestSeller.data.productBestSeller);
        }else{
          alert('Lấy dữ liệu sản phẩm bán chạy không thành công');
        }

        //
        const resProductBestSellerAsc = await fetch('https://huunghi.id.vn/api/product/getBestSalerProducts?by=asc',{
          headers : {
            "Content-Type" : "application/json",
            "Authorization" : `${JSON.parse(typeTokenLocal)} ${JSON.parse(accessTokenLocal)}`
          }
        ...
        if(resProductBestSellerAsc.ok){
          const resultResProductBestSellerAsc = await resProductBestSellerAsc.json();
          console.log(resultResProductBestSellerAsc.data.productBestSeller);
          setProductBestSellerAsc(resultResProductBestSellerAsc.data.productBestSeller);
        }else{
          alert('Lấy dữ liệu sản phẩm bán chạy không thành công');
        }


        const resPointEvalues = await fetch('https://huunghi.id.vn/api/function/getPointEvalues',{
          headers : {
            "Content-Type" : "application/json",
            "Authorization" : `${JSON.parse(typeTokenLocal)} ${JSON.parse(accessTokenLocal)}`
          }
        ...
        const resDataOrder = await fetch(`https://huunghi.id.vn/api/function/dataOrder?day=${dayData}`,{
          headers : {
            "Content-Type" : "application/json",
            "Authorization" : `${JSON.parse(typeTokenLocal)} ${JSON.parse(accessTokenLocal)}`
          }
        ...
        const resDiscount = await fetch(`https://huunghi.id.vn/api/function/getDiscountExpiringSoon`,{
          headers : {
            "Content-Type" : "application/json",
            "Authorization" : `${JSON.parse(typeTokenLocal)} ${JSON.parse(accessTokenLocal)}`
          }
  ...
  const recentOrders = [
    { id: '#DH20230025', customer: 'Trần Văn An', amount: '850.000₫', status: 'Đã giao', time: '15 phút trước' },
    { id: '#DH20230026', customer: 'Lê Thị Mai', amount: '1.250.000₫', status: 'Đang xử lý', time: '30 phút trước' },
    { id: '#DH20230027', customer: 'Phạm Văn Đức', amount: '550.000₫', status: 'Đang giao', time: '1 giờ trước' },
    { id: '#DH20230028', customer: 'Nguyễn Thị Hương', amount: '1.450.000₫', status: 'Đã hủy', time: '2 giờ trước' },
  ];
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Dashboard Bán Hàng</h1>
            <p className="text-sm text-gray-500">Tổng quan hoạt động cửa hàng</p>
          </div>
          <div className="flex gap-2 mt-4 md:mt-0">
            <select value={dayData} onChange={(e:any) => setDayData(e.target.value)} className="border rounded-lg px-3 py-2 text-sm bg-white">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
            ...
            <div className="space-y-4">
              {productBestSeller?.map((product,index) => (
                index < 5 && (
                  <div key={product.id_san_pham} className="flex items-center justify-between p-2 hover:bg-gray-50 rounded">
                    <div className="flex items-center">
                      <div className="w-10 h-10 bg-gray-200 rounded mr-3"><img className='rounded' src={`https://huunghi.id.vn/storage/products/${product.images[0].link_anh}`} alt="" /></div>
                      <div>
                        <h3 className="font-medium">{product.ten_san_pham}</h3>
                        <p className="text-xs text-gray-500">{product.gia_da_giam.toLocaleString('vi-VN')} đ</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-medium">{product.variant_orders_count} đã bán</p>
                      <p className={`text-xs ${product.product_variants_sum_so_luong < 10 ? 'text-red-500' : 'text-gray-500'}`}>
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
            ...
            <div className="space-y-4">
              {productBestSellerAsc?.map((product,index) => (
                index < 5 && (
                  <div key={product.id_san_pham} className="flex items-center justify-between p-2 hover:bg-gray-50 rounded">
                    <div className="flex items-center">
                      <div className="w-10 h-10 bg-gray-200 rounded mr-3"><img className='rounded' src={`https://huunghi.id.vn/storage/products/${product.images[0].link_anh}`} alt="" /></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              {listVoucher.map((coupon, index) => (
                <div key={index} className="flex items-center justify-between p-3 border border-gray-100 rounded-lg hover:bg-gray-50">
                  <div>
                    <div className="font-medium text-indigo-600">{coupon.ma_giam_gia}</div>
                    <div className="text-sm text-gray-500">{coupon.loai_giam_gia == 'phan_tram' ? coupon.gia_tri_giam+'%' : coupon.gia_tri_giam.toLocaleString('vi-VN')+' đ'  }</div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs text-amber-600">{new Date(coupon.ngay_het_han).toLocaleDateString('vi-VN')+ ' - ' + new Date(coupon.ngay_bat_dau).toLocaleDateString('vi-VN') }</div>
                    <div className="text-xs text-gray-500">Đã dùng: {coupon.order_count}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
            ...
            <div className="flex items-center justify-between mb-4 p-3 bg-gray-50 rounded-lg">
              <div className="text-center">
                <div className="text-3xl font-bold text-amber-500">{pointStart ? Number( pointStart.one * 1 + pointStart.two * 2 + pointStart.three * 3 + pointStart.four * 4 + pointStart.five * 5) / pointStart.all : 0}</div>
                ...
                <div className="text-xs text-gray-500">Dựa trên {pointStart.all} đánh giá</div>
              </div>
                ...
                <div key={order.id} className="flex items-center justify-between p-2 hover:bg-gray-50 rounded">
                  ...
                  <div className="text-right">
                    ...
                    <div className="flex items-center justify-end">
                      <span className={`text-xs px-2 py-1 rounded-full ${order.status === 'Đã giao' ? 'bg-green-100 text-green-800' :
