...
import { getAPIArticle, createdArticle, deleteArticle, editArticle } from "../services/articleService";
...
export default function AdminPostManagement() {
  ...
  const filteredPosts = posts.filter(post => {
    ...
    const contentMatch = contentFilter === "" || 
      post.noi_dung_bai_viet.toLowerCase().includes(contentFilter.toLowerCase());
    
    const statusMatch = statusFilter === '' || 
      post.trang_thai === statusFilter;
    
    const dateMatch = dateFilter === "" || 
      new Date(post.created_at).toLocaleDateString("vi-VN").includes(dateFilter);
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            >
              ...
              {[...new Set(posts.map(post => post.ten_bai_viet))].map((title, index) => (
          ...
          <div>
            ...
            >
              ...
              {[...new Set(posts.map(post => 
                post.noi_dung_bai_viet.replace(/<[^>]*>/g, '').substring(0, 50)
