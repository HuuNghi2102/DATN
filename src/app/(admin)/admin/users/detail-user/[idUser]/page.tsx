...
export default function UserDetailPage() {
    ...
    if (accessTokenLocal && typeTokenLocal && userLocal) {
      ...
      if (user.id_vai_tro == 1) {
        const resUser = await fetch(
          `https://huunghi.id.vn/api/user/getUser/${idUser}`,
          {
            headers: {
  ...
  const handleSave = async () => {
    ...
    const resChangeRoleUser = await fetch(
      `https://huunghi.id.vn/api/user/chnageRoleUser/${idUser}/idRole/${idRole}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
                    ...
                    {roles &&
                      roles.map((r, i) =>
                        r.id_vai_tro == user?.id_vai_tro ? (
                          <div
                            key={i}
                            className={`absolute -bottom-2 -right-2 px-3 py-1 rounded-full text-xs font-medium ${r.color} shadow-sm`}
                          >
                            {r.ten_vai_tro}
                          </div>
                        ) : (
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div>
                    <h3 className="text-md font-medium text-gray-700 mb-4 pb-2 border-b border-gray-100">
                      Thông tin cá nhân
                    </h3>
                  ...
                  <div>
                    ...
                    <div className="space-y-5">
                        ...
                        {isEditing ? (
                          ...
                          roles.map((r, i) =>
                            r.id_vai_tro == user?.id_vai_tro ? (
                              <div
                                key={i}
                                className={`px-4 py-2.5 rounded-lg inline-flex items-center ${r.color}`}
                              >
                                {r.ten_vai_tro}
                              </div>
                            ) : (
                      ...
                      <div>
                        ...
                        <div
                          className={`px-4 py-2.5 rounded-lg inline-flex items-center ${
                            user?.trang_thai === 1
                              ? "bg-green-100 text-green-800"
                              : "bg-red-100 text-red-800"
                          }`}
