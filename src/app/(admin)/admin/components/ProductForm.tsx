...
export default function ProductForm({ product, categories, typeToken, accessToken }: Props) {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        setFormData(prev => ({
            ...prev,
            [name]: name === 'gia_chua_giam' || name === 'gia_da_giam' || name === 'giam_gia'
                ? Number(value)
                : value
        }));
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // fetchImage
            const resUpdateProduct = await fetch(`https://huunghi.id.vn/api/product/updateProduct/${product.duong_dan}`, {
                method: "PUT",
                headers: {
            ...
            <div className="p-6">
                <form id="productForm" className="space-y-6" onSubmit={handleSubmit}>
                    ...
                    <div>
                        ...
                        >
                            {categories.map((c, i) => (
                                <option key={i} value={c.id_loai_san_pham}>{c.ten_loai}</option>
                            ))}
                        </select>
