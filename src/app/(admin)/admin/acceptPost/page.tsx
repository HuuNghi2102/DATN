    ...
    <div className="min-h-screen bg-gray-100 text-gray-900 font-sans p-8">
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200">
          ...
          <span className="inline-block px-3 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">{posts.length} bài viết chờ xử lý</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-100 text-left">
                {["ID", "Tiêu đề", "Danh mục", "Người đăng", "Ngày đăng", "Trạng thái", "Thao tác"].map((th, i) => (
            ...
            <tbody>
              {posts.map((post, idx) => (
                <tr key={post.id} className="hover:bg-gray-50">
                  <td className="px-4 py-4 border-b border-gray-200">{post.id}</td>
                  ...
                  <td className="px-4 py-4 border-b border-gray-200">{post.category}</td>
                  <td className="px-4 py-4 border-b border-gray-200">{post.author}</td>
                  <td className="px-4 py-4 border-b border-gray-200">{post.date}</td>
                  <td className="px-4 py-4 border-b border-gray-200">
                    <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${statusColor(post.status)}`}>{post.status}</span>
                  </td>
                  <td className="px-4 py-4 border-b border-gray-200">
                    <div className="flex gap-2">
                      {["eye", "check", "times", "pencil-alt"].map((icon, i) => (
                        <button key={i} className="w-8 h-8 flex items-center justify-center border border-gray-300 rounded hover:bg-gray-100 hover:text-gray-700 text-gray-500">
        ...
        <div className="flex justify-between items-center px-6 py-4 border-t border-gray-200">
          <div className="text-sm text-gray-500">Hiển thị 1-5 của 15 bài viết</div>
          <div className="flex gap-1">
            {["chevron-left", "1", "2", "3", "chevron-right"].map((label, i) => (
              <button
                key={i}
                className={`w-9 h-9 rounded flex items-center justify-center border text-sm ${label === "1" ? "bg-indigo-600 text-white border-indigo-600" : "border-gray-300 text-gray-700 hover:bg-gray-100"}`}
              >
