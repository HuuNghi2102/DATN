    ...
    if (accessTokenLocal && typeTokenLocal && userLocal) {
      ...
      if (user.id_vai_tro == 1) {
        ...
        try {
          ...
          //fectchCategory 
          const resCategory = await fetch('https://huunghi.id.vn/api/categoryProduct/listCategoryProduct',{
            headers : {
              "Content-Type" : "application/json",
              "Authorization" : `${JSON.parse(typeTokenLocal)} ${JSON.parse(accessTokenLocal)}`,
            }
  ...
  const addProduct = async () => {
    ...
    if (accessTokenLocal && typeTokenLocal && userLocal) {
      ...
      if (user.id_vai_tro == 1) {
        ...
        newProduct.append('variant[quantity]', product.variant.quantity.toString());
        setIsLoading(true);
        try {
          // fetchAddProduct
          const resAddProduct = await fetch(`https://huunghi.id.vn/api/product/addProduct`, {
            method : "POST",
            headers: {
              // "Content-Type": "application/json",
              "Authorization": `${JSON.parse(typeTokenLocal)} ${JSON.parse(accessTokenLocal)}`
          ...
          if (result.status == true) {
            ...
            if(result.errors){
              ...
              setErrProduct({
                ...
                nameColor : err['variant.nameColor'] ? err['variant.nameColor'][0] : "",
                codeColor : err['variant.codeColor'] ? err['variant.codeColor'][0] : "",
                idSize : err['variant.idSize'] ? err['variant.idSize'][0] : "",
                quantity : err['variant.nameColor'] ? err['variant.nameColor'][0] : ""
              })
              return;
            }
            alert(result.message)
      ...
      <div className="card-body p-6">
        <form id="productForm" onSubmit={handleSubmit}>
          ...
          <div className="form-group mb-5">
            ...
            />
            {errProduct.name && <p className="text-red-500 text-sm mt-1">{errProduct.name}</p>}
          </div>
          <div className="form-group mb-5">
            <label className="form-label block text-sm font-medium text-gray-700 mb-1">Đường dẫn</label>
            <input
              value={product.slug}
               onChange={(e:any) => {setProduct({...product, slug : e.target.value})}}
              type="text"
              className="form-control w-full px-3 py-2 border border-gray-300 rounded focus:border-primary focus:ring-1 focus:ring-primary-light outline-none text-sm"
              placeholder="Ví dụ: duong-dan-san-pham"
            />
            {errProduct.slug && <p className="text-red-500 text-sm mt-1">{errProduct.slug}</p>}
          </div>
          <div className="form-group mb-5">
            ...
            ></textarea>
            {errProduct.description && <p className="text-red-500 text-sm mt-1">{errProduct.description}</p>}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-5">
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e:any) => {setProduct({...product, price_old : e.target.value})}}
                type="number"
                className="form-control w-full px-3 py-2 border border-gray-300 rounded focus:border-primary focus:ring-1 focus:ring-primary-light outline-none text-sm"
                placeholder="0"
                min="1"
              />
              {errProduct.price_old && <p className="text-red-500 text-sm mt-1">{errProduct.price_old}</p>}
            </div>
            <div className="form-group">
              <label className="form-label block text-sm font-medium text-gray-700 mb-1">Giảm giá (%)</label>
              <input
                value={product.percent}
                onChange={(e:any) => {setProduct({...product, percent : e.target.value != 0 ? e.target.value : '' })}}
                type="number"
                className="form-control w-full px-3 py-2 border border-gray-300 rounded focus:border-primary focus:ring-1 focus:ring-primary-light outline-none text-sm"
                placeholder="0"
                min="1"
              />
              {errProduct.percent && <p className="text-red-500 text-sm mt-1">{errProduct.percent}</p>}
            </div>
          </div>
          ...
          <div className="form-group mb-5">
            ...
            <input
              ...
              onChange={(e:any) => {setProduct({...product, price_sale : e.target.value})}}
              type="number"
              className="form-control w-full px-3 py-2 border border-gray-300 rounded focus:border-primary focus:ring-1 focus:ring-primary-light outline-none text-sm"
              placeholder="0"
              min="1"
            />
            {errProduct.price_sale && <p className="text-red-500 text-sm mt-1">{errProduct.price_sale}</p>}
          </div>
          <div className="form-group mb-5">
            ...
            <select className="form-control w-full px-3 py-2 border border-gray-300 rounded focus:border-primary focus:ring-1 focus:ring-primary-light outline-none text-sm">
              ...
              {listCategory.map((cate,index)=>(
                <option 
                  ...
                  onChange={(e:any) => {setProduct({...product, idCate : e.target.value})}}
                >
                  {cate.ten_loai}
                </option>
              ))}
            </select>
            {errProduct.idCate && <p className="text-red-500 text-sm mt-1">{errProduct.idCate}</p>}
          </div>
          ...
          <div className="form-group mb-5">
            ...
            {errProduct.images && <p className="text-red-500 text-sm mt-1">{errProduct.images}</p>}
              ...
              <div className="inventory-item bg-gray-100 p-4 rounded border border-dashed border-gray-300 mb-4 relative">
                <div className="inventory-grid grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div className="form-group">
                    ...
                    <select onChange={(e:any) => {setProduct({...product , variant : {...product.variant ,idSize : e.target.value }});console.log(e.target.value)}} className="form-control w-full px-3 py-2 border border-gray-300 rounded focus:border-primary focus:ring-1 focus:ring-primary-light outline-none text-sm">
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e:any) => {setProduct({...product , variant : { ...product.variant, quantity : e.target.value}})}}
                      type="number"
                      className="form-control w-full px-3 py-2 border border-gray-300 rounded focus:border-primary focus:ring-1 focus:ring-primary-light outline-none text-sm"
                      placeholder="0"
                      min="0"
                    />
                    {errProduct.quantity&& <p className="text-red-500 text-sm mt-1">{errProduct.quantity}</p>}
                  </div>
                </div>
              </div>
            </div>
