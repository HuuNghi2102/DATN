  ...
  const fetchDefaultData = async () => {
    ...
    if (accessTokenLocal && typeTokenLocal && userLocal) {
      ...
      if (user.id_vai_tro == 3) {
        ...
        const resOrders = await fetch(`https://huunghi.id.vn/api/order/getAllOrders?status=${selectedStatus}&page=${currentPage}&codeOrder=${searchQuery}`, {
          headers: {
                ...
                {orders.map((order) => (
                  <React.Fragment key={order.id_don_hang}>
                    <tr className="border-b hover:bg-gray-50">
                      <td className="px-6 py-4 font-medium">#DH{order.id_don_hang}</td>
