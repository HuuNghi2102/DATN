...
const OrderManager = () => {
  ...
  const setTotalCateOrder = (status:string) => {
    ...
    switch(status){
      ...
      case 'dang_giao':
        ...
        setCountWaitGetOrder(countWaitGetOrder - 1);
        break
      case 'hoan_hang': break;
      case 'giao_thanh_cong': break;
      case 'da_huy':
  ...
  const fetchDefaultData = async () => {
    ...
    if (accessTokenLocal && typeTokenLocal && userLocal) {
      ...
      if (user.id_vai_tro == 1) {
        ...
        const resOrders = await fetch(`https://huunghi.id.vn/api/order/getAllOrders?status=${activeTab}&page=${currentPage}&codeOrder=${searchQuery}`, {
          headers: {
  ...
  const getBadgeClass = (status:string) => {
    ...
    switch (status) {
      case 'cho_xac_nhan': return `${base} bg-yellow-100 text-yellow-800`;
      case 'chu_y': return `${base} bg-orange-100 text-orange-800`;
      case 'dang_chuan_bi_hang': return `${base} bg-blue-100 text-blue-800`;
      case 'cho_lay_hang': return `${base} bg-indigo-100 text-indigo-800`;
      case 'dang_giao': return `${base} bg-purple-100 text-purple-800`;
      case 'hoan_hang': return `${base} bg-pink-100 text-pink-800`;
      case 'giao_thanh_cong': return `${base} bg-green-100 text-green-800`;
      case 'da_huy': return `${base} bg-red-100 text-red-800`;
  ...
  const getStatusActions = (status:string,checkReturn:number) => {
    ...
    switch (status) {
      ...
      case 'dang_chuan_bi_hang':
        return [
          { action: 'ready', icon: 'box', color: 'blue', codeNextStatus : "cho_lay_hang", nextStatus: 'Chờ lấy hàng', title: 'Đã chuẩn bị xong' },
          { action: 'cancel', icon: 'times', color: 'red', codeNextStatus : "da_huy" , nextStatus: 'Đã hủy', title: 'Hủy đơn hàng' }
        ];
      case 'cho_lay_hang':
        return [
  ...
  const statusTabs = [
    { status: 'Tất cả đơn hàng',codeStatus : '', icon: 'list' , totalOrder: totalOrder },
    { status: 'Chờ xác nhận',codeStatus : 'cho_xac_nhan',icon: 'hourglass-half', totalOrder: countWaitConfirm },
    { status: 'Chú ý',codeStatus : 'chu_y',icon: 'exclamation' , totalOrder: countAttention},
    { status: 'Đang chuẩn bị hàng',codeStatus :'dang_chuan_bi_hang',icon: 'box' , totalOrder: countPrepareOrder},
    { status: 'Chờ lấy hàng',codeStatus : 'cho_lay_hang',icon: 'boxes' , totalOrder: countWaitGetOrder },
    { status: 'Đang giao', codeStatus : 'dang_giao',icon: 'truck' , totalOrder: countInProgress },
    { status: 'Đang hoàn hàng',codeStatus : 'hoan_hang',icon: 'undo' , totalOrder: countReturn },
    { status: 'Giao thành công',codeStatus : 'giao_thanh_cong', icon: 'check-circle' , totalOrder: countSuccess },
    { status: 'Đã hủy',codeStatus : 'da_huy',icon: 'times-circle' , totalOrder: countDestroy }
  ];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">Tổng quan và quản lý tất cả đơn hàng của cửa hàng</p>
          </div>
          
          <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
            <div className="relative flex-1">
              <form onSubmit={(e:any)=> {e.preventDefault();handleSearch()}}>
                  <input
                    type="text"
          ...
          <div className="bg-white p-4 md:p-6 rounded-lg shadow-md mb-6 border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1">Danh sách sản phẩm</label>
                <textarea 
          ...
          <div className="bg-white p-4 rounded-lg shadow border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold mt-1">{countInProgress + countReturn}</p>
              </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {orders.length > 0 ? (
                  orders.map(order => (
                    <tr key={order.id_don_hang} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-indigo-600">#DH{order.id_don_hang}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">{order.ten_nguoi_nhan}</div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={getBadgeClass(order.trang_thai_don_hang)}>
                          {returStatus(order.trang_thai_don_hang)}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end items-center space-x-2">
                          {getStatusActions(order.trang_thai_don_hang,order.kiem_tra_hoan_hang).map((action, index) => (
                            <button
                              key={index}
                              onClick={() => action.action === 'print' 
                                ? handlePrint(order.id_don_hang) 
                                : updateStatus(order.id_don_hang, action.codeNextStatus != undefined ? action.codeNextStatus : "")}
                              className={`p-2 rounded-full ${
                                ...
                                action.color === 'purple' ? 'text-purple-600 hover:bg-purple-50' :
                                action.color === 'pink' ? 'text-pink-600 hover:bg-pink-50' :
                          ...
                          <Link href={`/admin/orders/detai-order/${order.id_don_hang}`}>
                            <button
                  ...
                  <tr>
                    <td colSpan="7" className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center justify-center text-gray-400">
                        ...
                        <p className="text-sm mt-1">Không có đơn hàng nào phù hợp với tiêu chí tìm kiếm</p>
                        <button 
                          onClick={() => {
                            setActiveTab('');
                            setSearchQuery('');
